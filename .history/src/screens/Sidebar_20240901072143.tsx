    ...
    const fetchUserDetails = async () => {
      try {
        ...
        if (user) {
          ...
          if (userData) {
            setUserDetails({
              name: userData.name || '', // Default to '' if name is not available
              role: userData.role || 'Pharmacist', // Default to 'Pharmacist' if role is not available
            });
          }
        }
      } catch (error) {
  ...
  return (
    <View style={styles.sidebar}>
      ...
      <TouchableOpacity style={styles.closeButton} onPress={onClose}>
        <Icon name="close" size={24} color="#000" /> {/* Changed to black */}
      </TouchableOpacity>
      ...
      <View style={styles.linksContainer}>
        ...
        >
          <Icon name="list" size={20} color="#000" /> {/* Changed to black */}
          <Text style={styles.linkText}>Inventory</Text>
        </TouchableOpacity>
        {/* Add other links here */}
        <TouchableOpacity style={styles.link}>
          <Icon name="search" size={20} color="#000" /> {/* Changed to black */}
          <Text style={styles.linkText}>Track Orders</Text>
        </TouchableOpacity>
        <TouchableOpacity style={styles.link}>
          <Icon name="bell" size={20} color="#000" /> {/* Changed to black */}
          <Text style={styles.linkText}>Follow Ups</Text>
        </TouchableOpacity>
        <TouchableOpacity style={styles.link}>
          <Icon name="exclamation-triangle" size={20} color="#000" /> {/* Changed to black */}
          <Text style={styles.linkText}>Complaints</Text>
        </TouchableOpacity>
        <TouchableOpacity style={styles.link}>
          <Icon name="phone" size={20} color="#000" /> {/* Changed to black */}
          <Text style={styles.linkText}>Contacts</Text>
        </TouchableOpacity>
        <TouchableOpacity style={styles.link}>
          <Icon name="calendar" size={20} color="#000" /> {/* Changed to black */}
      ...
      <TouchableOpacity style={styles.logoutButton} onPress={onLogout}>
        <Icon name="sign-out" size={20} color="#000" /> {/* Changed to black */}
...
const styles = StyleSheet.create({
  ...
  logoutButton: {
    ...
    paddingVertical: 15,
    borderTopWidth: 1,
    borderTopColor: '#000', // Changed to black
  },
  logoutText: {
    fontSize: 16,
    color: '#000', // Changed to black
